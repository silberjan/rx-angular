...
const setupTestComponent = () => {
  TestBed.configureTestingModule({
    declarations: [LetDirectiveAllTemplatesTestComponent, ScheduleOnDirective],
    providers: [
...
describe('LetDirective when template binding with all templates', () => {
  ...
  it('should have `ngTemplateContextGuard` defined', () => {
    expect(ScheduleOnDirective.ngTemplateContextGuard).toBeDefined();
    expect(
      ScheduleOnDirective.ngTemplateContextGuard({} as ScheduleOnDirective<any>, {})
    ).toBe(true);
  });
});
